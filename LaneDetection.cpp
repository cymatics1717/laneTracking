...
bool LaneDetection::initialize_variable(cv::Mat& img_src) {
...
bool LaneDetection::initialize_Img(cv::Mat& img_src) {

	// Loading an input image
	if (img_src.empty()) {
        std::cout << "Err: Cannot find the input image: "  << std::endl;
		return false;
	}
